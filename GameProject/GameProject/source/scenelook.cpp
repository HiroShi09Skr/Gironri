			...
			for (x = cx - cr; x < cx + cr; x++) {
				if (0 <= x && x < LIGHTMAP_W) {		// マップ内に入っているか
					...
					case 0:
						...
						// もともとのアルファ値を加味して穴をあける
						//int base_alpha = mapLight[y * LIGHTMAP_W + x];
						//a -= (ALPHA_VALUE - base_alpha);
						//if (a < 0) { a = 0; }
						//mapLight[y * LIGHTMAP_W + x] = a;
...
void SceneLook::CheckPoint(float px, float py)
{
	...
	{
		UniqueNoAnsCnt++;	// gameChapterが2の時、ある特定の箇所を睨んだ場合、加算（+1）
	}
...
void SceneLook::lookTalk()
{
	...
	if (gameChapter == 2 && flgAnswer == 3 && UniqueNoAnsNum > 0)
	{
		...
		else if (msize[UniqueNoAnsCnt - 1] > drawCnt)		// 部分表示
		{
		...
		else if (msize[UniqueNoAnsCnt - 1] <= drawCnt)		// 全文表示
		{
	...
	else
	{
		...
		// 1行目
		else if (messageByte[0][drawCnt1] != '\0')		// 部分表示
		{
		...
		{
			// 2行目
			if (messageByte[1][drawCnt2] != '\0')		// 部分表示
			{
...
void SceneLook::Process()
{
	...
	case 2:
		...
		if (gPad.gTrg & PAD_INPUT_1) {
			if (CheckSoundMem(seBGM)) { StopSoundMem(seBGM); }
		...
		case 2:
			...
			else
			{
				...
				//splScale(gPad.xinput_pad1);	// スポットライトのサイズ変更
				...
				// 正解・不正解・特殊不正解の時、全文表示から60フレーム後、自動で選択時の文が表示される
			...
			if (lightCnt == LIGHTMAP_W * LIGHTMAP_H)
			{
				...
				SceneBase* scene = new SceneTalk(gameChapter + 51);
				//SceneBase* scene = new SceneLook(gameChapter + 1);
				ChangeScene(scene);
			}
...
void SceneLook::Draw()
{
	...
	for (i = 0; i < tblLPSize; i++)
	{
		if (tblLookPoint[i].chapter == gameChapter)
		{
			if (tblLookPoint[i].hittype == 0)
			{
				DrawCircle(tblLookPoint[i].x, tblLookPoint[i].y, tblLookPoint[i].r, GetColor(0, 0, 255), 0);
			}
			else
			{
				w = tblLookPoint[i].x + tblLookPoint[i].w;
				h = tblLookPoint[i].y + tblLookPoint[i].h;
				DrawBox(tblLookPoint[i].x, tblLookPoint[i].y, w - 1, h - 1, GetColor(0, 0, 255), FALSE);
			}
		}
	}
	...
	for (i = 0; i < 6; i++)
	{
		if (tblLookEaster[gameChapter][i].hittype == 0)
		{
			DrawCircle(tblLookEaster[gameChapter][i].x, tblLookEaster[gameChapter][i].y, tblLookEaster[gameChapter][i].r, GetColor(0, 0, 255), 0);
		}
		else
		{
			w = tblLookEaster[gameChapter][i].x + tblLookEaster[gameChapter][i].w;
			h = tblLookEaster[gameChapter][i].y + tblLookEaster[gameChapter][i].h;
			DrawBox(tblLookEaster[gameChapter][i].x, tblLookEaster[gameChapter][i].y, w - 1, h - 1, GetColor(0, 0, 255), FALSE);
		}
	}*/
	...
	//DrawFormatString(100, 100, GetColor(255, 255, 255), "SceneLook!!");
	//DrawFormatString(100, 150, GetColor(255, 255, 255), "%d", spotlightSize);
	...
	//DrawFormatString(100, 200, GetColor(0, 0, 255), "%d", lightCnt);
	...
	//DrawFormatString(100, 700, GetColor(255, 255, 255), "gameChapter + 51：%d", gameChapter + 51);
