...
void Deck::DrawCard(int flg_gf)
{
	...
	case 0:		// 議論パート
		...
		for (j = 0; j < cards.size(); j++)
		{
			...
			//DrawFormatString(cards[i].pos.x, cards[i].pos.y, GetColor(0, 255, 0), "%d", cards[i].num);
		}
		break;
	case 1:		// 反論パート
		// カード展開中
		...
		else
		{
			for (i = 0; i < cards.size(); i++)
			{
				...
				{
					DrawRotaGraph(cards[i].pos.x, cards[i].pos.y, cards[i].scale, 0.0f, cards[i].cg, TRUE, 0);
					//DrawFormatString(cards[i].pos.x, cards[i].pos.y, GetColor(255, 0, 0), "%d", cards[i].num);
				}
				// 選択していないカードは小さく薄く表示
				else
				{
