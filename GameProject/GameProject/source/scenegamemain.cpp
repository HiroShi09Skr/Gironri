		...
		else
		{
			if (isPause != true)
			{
				...
				case 0:
					...
					//反論パートへ（開発用）
					/*
					if (gPad.gTrg & PAD_INPUT_3 && !myDeck.cards.empty()) {
						flg_gameflow = 1;
					}*/
...
void SceneGameMain::Draw()
{
	...
	//開発用
	//DrawFormatString(100, 700, GetColor(255, 255, 255), "text_flg;：%d", text_flg);
	//DrawFormatString(100, 600, GetColor(255, 255, 255), "gameChapter：%d", gameChapter * 100 + myDeck.cards[myDeck.cards.size() / 2].num * 10 + myDeck.cards[myDeck.cards.size() / 2].num * 1);
