		...
		else
		{
			...
			else
			{
				...
				case 0:
								...
								else
								{
									...
									//証言
										...
										if (have_card < deckMax[gameChapter]) {
											...
											{
												myDeck.GetCard(words[i].numName, cgStockCard[gameChapter][words[i].numName][words[i].testimonyCnt], words[i].x, words[i].y, words[i]._scale, words[i].testimonyCnt);
												have_card++;
												if (tutorial_flg == 6)
												{
													isPause = TRUE;
					...
					//反論パートへ（開発用）
					/*
					if (tutorial_flg == 8 && gPad.gTrg & PAD_INPUT_2 && !myDeck.cards.empty()) {
						flg_gameflow = 1;
					}*/
...
void SceneGameTutorial::Draw()
{
	...
	switch (flg_gameflow) {
	case 0:
		//DrawFormatString(100, 100, GetColor(255, 255, 255), "Giron");

		//DrawFormatString(100, 100, GetColor(255, 255, 255), "SceneGameMain!!");
		//word.MyDrawModiGraph(word._mView, VGet(word.X(), word.Y(), 0), 1.f, 3, 64, 64, cgWord,0);
		for (int i = 0; i < WORD_MAX; i++)
		{
			if (words[i].use != 0)
			{
				if (words[i].wordType == 1) DrawRotaGraph(words[i].x, words[i].y, words[i]._scale, 0, words[i].cgName, 1, 0);
				if (words[i].wordType == 2) DrawRotaGraph(words[i].x, words[i].y, words[i]._scale, 0, cgTestimony[gameChapter][words[i].numName][words[i].testimonyCnt], 1, 0);
			}
			//DrawFormatString(100, 300 + i * 50, GetColor(255, 255, 255), "words[%d].x :%d, words[%d].y :%d, words[%d].use :%d", i, words[i].x, i, words[i].y, i, words[i].use);
		}

		//if (flg_shield == 1) { DrawLine(0, PLAYER_GET_LINE, SCREEN_W, PLAYER_GET_LINE, GetColor(255, 255, 255), TRUE); }

		//DrawCircle(word.X(), word.Y(), 1, GetColor(255, 255, 255),1,1 );
		//(word.X(),word.Y(), cgWord, 0);
		...
		break;
	case 1:
		//DrawFormatString(100, 100, GetColor(255, 255, 255), "Hanron");
		break;
	case 2:
		if (myDeck.flg_ans == 1)
		{
			//DrawFormatString(100, 100, GetColor(255, 255, 255), "Correct!");
		}
		if (myDeck.flg_ans == 2)
		{
			//DrawFormatString(100, 100, GetColor(255, 255, 255), "Incorrect!");
		}
		break;
	}
	...
	//DrawFormatString(1500, 100, GetColor(255, 255, 255), "HP%d", gHP);
	//DrawFormatString(1500, 200, GetColor(255, 255, 255), "AT%d", player.AT);

	// 審議会メンバー
	/*
	for (i = 0; i < ENEMY_MAX; i++)
	{
		DrawRotaGraph(gikai[i].X, gikai[i].Y, 0.8f, 0.0f, gikai[i].cg, TRUE, 0);
	}*/

	gEffect.DrawEffect();
	//DrawFormatString(1000, 600, GetColor(255, 255, 255), "lockCnt：%d", lockCnt);
	...
	if (isPause == true)
	{
		//DrawFormatString(100, 1000, GetColor(255, 255, 255), "ポーズ");
		//操作説明
	...
	//DrawFormatString(1300, 850, GetColor(255, 255, 255), "tutorial_flg%d", tutorial_flg);
	//DrawFormatString(1300, 900, GetColor(255, 255, 255), "tutorial_cnt%d", tutorial_cnt);

	//開発用
	//DrawFormatString(100, 600, GetColor(255, 255, 255), "cutInCount：%d", cutInCount / 3);
	//DrawFormatString(100, 600, GetColor(255, 255, 255), "ゲームフロー%d", flg_gameflow);	
	//DrawFormatString(100, 700, GetColor(255, 255, 255), "シナリオ証言数%d", scenarioTestimonyCnt);
	//DrawFormatString(100, 800, GetColor(255, 255, 255), "カットインカウント%d", cutInCount);
	//DrawFormatString(100, 900, GetColor(255, 255, 255), "ガードカウント%d", gardCount);
}
